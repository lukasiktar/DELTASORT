...
os.makedirs("DELTASORT_image_processing/results", exist_ok=True)
...
for i, path in enumerate(image_paths):
    ...
    cv2.imwrite(f"DELTASORT_image_processing/results/channel{i}_normalized.png", img_norm)

    # Apply JET colormap (visualization only)
    img_jet = cv2.applyColorMap(img_norm, cv2.COLORMAP_JET)
    img_jets.append(img_jet)
    cv2.imwrite(f"DELTASORT_image_processing/results/channel{i}_jet.png", img_jet)
...
cv2.imwrite("DELTASORT_image_processing/results/diff_gray_jet01.png", diff_gray_jet01)
...
cv2.imwrite("DELTASORT_image_processing/results/diff_thresh_jet01.png", thresh_diff)
...
cv2.imwrite("DELTASORT_image_processing/results/diff_gray_jet12.png", diff_gray_jet12)
...
cv2.imwrite("DELTASORT_image_processing/results/diff_thresh_jet12.png", thresh_diff)
...
cv2.imwrite("DELTASORT_image_processing/results/diff_gray_jet23.png", diff_gray_jet23)
...
cv2.imwrite("DELTASORT_image_processing/results/diff_thresh_jet23.png", thresh_diff)
...
cv2.imwrite("DELTASORT_image_processing/results/diff_gray_jet34.png", diff_gray_jet34)
...
cv2.imwrite("DELTASORT_image_processing/results/diff_thresh_jet34.png", thresh_diff)
