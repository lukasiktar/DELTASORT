...
def connect_basler(num_cameras):
    """ 
    Connect Basler cameras and set the parameters.
    """
    #Check the available devices and store them
    cameras = []
    ...
    for i in range(num_cameras):
        ...
        cam.TriggerSource.SetValue("Line1") # Or "Software"
...
def connect_realsense():
    """ 
    Connect RealSense camera and set the parameters.
    """
...
def main():
    duration = 20  # The numver of seconds for recording
    ...
    try:
        while (time.time() - start_time) < duration:
            ...
            for i, cam in enumerate(basler_cams):
                ...
                if grab_result.GrabSucceeded():
                    ...
                    if i == 0:
                        ...
                        img = cv2.warpPerspective(img, H, (height, width))
                        #ROI
                        img = img[200:height-100,400:width-450]
                    else:
                        #ROI
                        img = img[200:height-100,400:width-450]
            ...
            if len(basler_images) > 0:
                cv2.imwrite(f"images_basler_1/{timestamp}_channel_{channel_count}.jpg", basler_images[0], [cv2.IMWRITE_JPEG_QUALITY, 95])
            if len(basler_images) > 1:
                cv2.imwrite(f"images_basler_2/{timestamp}channel_{channel_count}.jpg", basler_images[1], [cv2.IMWRITE_JPEG_QUALITY, 95])
